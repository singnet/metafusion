...
# Copied and modified from diffusers.pipelines.stable_diffusion_xl.pipeline_stable_diffusion_xl_img2img.retrieve_latents
        ...
        **kwargs
        ...
        Args:
			...
			sample_mode (`str`, *optional*):
				control latents initialisation for the inpaint area, can be one of sample, argmax, mean, random
        Examples:
...
#				# replace non-masked latents with original latents
#                if latent_mask is not None:
#                    latents = torch.lerp(non_paint_latents / self.vae.config.scaling_factor, latents, latent_mask)
#					# now we don't actually have noise outside of the mask 
#                noise_pred = torch.lerp(torch.zeros_like(noise_pred), noise_pred, latent_mask)
        ...
        if not output_type == "latent":
            ...
            l1 = torch.lerp(non_paint_latents, latents, latent_mask)
            l1 = self.denormalize(l1)
            image = self.vae.decode(l1, return_dict=False)[0]
            if self.debug_save:
